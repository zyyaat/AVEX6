...
func HandleDriverPickedUp(w http.ResponseWriter, r *http.Request) {
        ...
        if realtime.Enabled() {
                var pickupPhoto sql.NullString
                shared.DB.QueryRow("SELECT pickup_photo_url FROM orders WHERE id = $1", orderID).Scan(&pickupPhoto)
                if !pickupPhoto.Valid || pickupPhoto.String == "" {
                        shared.WriteErr(w, 400, "يجب رفع صورة الاستلام من المطعم أولاً")
                        return
                }
        }
...
func HandleDriverArrived(w http.ResponseWriter, r *http.Request) {
        ...
        if status.String == "picked_up" {
                ...
                shared.DB.Exec(`INSERT INTO order_status_history (id, order_id, status, changed_by) VALUES ($1, $2, 'on_the_way', $3)`, uuid.New().String(), orderID, c.DriverID)
                realtime.NotifyDriverOrderUpdate(c.DriverID, map[string]interface{}{"order_id": orderID, "status": "on_the_way"})
                realtime.NotifyAdminZoneUpdate(map[string]interface{}{"order_id": orderID, "status": "on_the_way"})
        }
...
func HandleDriverDelivered(w http.ResponseWriter, r *http.Request) {
        ...
        shared.DB.Exec(`INSERT INTO order_status_history (id, order_id, status, changed_by) VALUES ($1, $2, 'delivered', $3)`, uuid.New().String(), orderID, c.DriverID)
        ...
        realtime.NotifyDriverOrderUpdate(c.DriverID, map[string]interface{}{"order_id": orderID, "status": "delivered"})
        realtime.NotifyAdminZoneUpdate(map[string]interface{}{"order_id": orderID, "status": "delivered"})
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true, "status": "delivered", "earnings": driverFee.Float64})
}

// HandleDriverRequestPhotoUploadURL issues a presigned upload URL (Replit object storage)
// for the driver's pickup/delivery photo capture flow.
func HandleDriverRequestPhotoUploadURL(w http.ResponseWriter, r *http.Request) {
        c := shared.GetDriver(r)
        if c == nil {
                shared.WriteErr(w, 401, "غير مصرح")
                return
        }
        if !storage.Enabled() {
                shared.WriteErr(w, 503, "تخزين الملفات غير مفعل")
                return
        }
        uploadURL, objectPath, err := storage.GetUploadURL("order-photos")
        if err != nil {
                shared.WriteErr(w, 500, "تعذر إنشاء رابط الرفع")
                return
        }
        shared.WriteJSON(w, 200, map[string]interface{}{"uploadUrl": uploadURL, "objectPath": objectPath})
}

// HandleDriverUploadPickupPhoto stores the mandatory pickup photo URL for an order.
// The "picked-up" transition is blocked (when realtime is enabled) until this is called.
func HandleDriverUploadPickupPhoto(w http.ResponseWriter, r *http.Request) {
        c := shared.GetDriver(r)
        if c == nil {
                shared.WriteErr(w, 401, "غير مصرح")
                return
        ...
        shared.DB.QueryRow("SELECT status FROM orders WHERE id = $1 AND driver_id = $2", orderID, c.DriverID).Scan(&status)
        if !status.Valid {
                shared.WriteErr(w, 404, "الطلب غير موجود")
                return
        }
        var body struct {
                URL string `json:"url"`
        ...
        if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.URL == "" {
                shared.WriteErr(w, 400, "رابط الصورة مطلوب")
                return
        }
        shared.DB.Exec("UPDATE orders SET pickup_photo_url = $1 WHERE id = $2", body.URL, orderID)
        shared.DB.Exec(`INSERT INTO order_photos (id, order_id, type, url, uploaded_by) VALUES ($1, $2, 'pickup', $3, $4)`,
                uuid.New().String(), orderID, body.URL, c.DriverID)
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}

// HandleDriverUploadDeliveryPhoto stores an optional delivery confirmation photo.
func HandleDriverUploadDeliveryPhoto(w http.ResponseWriter, r *http.Request) {
        c := shared.GetDriver(r)
        if c == nil {
                shared.WriteErr(w, 401, "غير مصرح")
                return
        ...
        shared.DB.Exec("UPDATE orders SET delivery_photo_url = $1 WHERE id = $2 AND driver_id = $3", body.URL, orderID, c.DriverID)
        shared.DB.Exec(`INSERT INTO order_photos (id, order_id, type, url, uploaded_by) VALUES ($1, $2, 'delivery', $3, $4)`,
                uuid.New().String(), orderID, body.URL, c.DriverID)
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}

// HandleDriverZoneTransferRequest lets a driver ask to move to a different zone.
// Only admin can approve/reject — see admin package.
func HandleDriverZoneTransferRequest(w http.ResponseWriter, r *http.Request) {
        c := shared.GetDriver(r)
        if c == nil {
                shared.WriteErr(w, 401, "غير مصرح")
                return
        ...
        if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.ToZoneID == "" {
                shared.WriteErr(w, 400, "المنطقة المطلوبة غير محددة")
                return
        }
        var pending int
        shared.DB.QueryRow("SELECT COUNT(*) FROM zone_transfer_requests WHERE driver_id = $1 AND status = 'pending'", c.DriverID).Scan(&pending)
        if pending > 0 {
                shared.WriteErr(w, 400, "لديك طلب نقل قيد المراجعة بالفعل")
                return
        }
        var currentZone sql.NullString
        shared.DB.QueryRow("SELECT zone_id FROM drivers WHERE id = $1", c.DriverID).Scan(&currentZone)
        id := uuid.New().String()
        shared.DB.Exec(`INSERT INTO zone_transfer_requests (id, driver_id, from_zone_id, to_zone_id, reason) VALUES ($1, $2, $3, $4, $5)`,
                id, c.DriverID, currentZone.String, body.ToZoneID, body.Reason)
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true, "id": id})
}

// HandleDriverGetZoneTransferRequests returns the driver's own transfer request history.
func HandleDriverGetZoneTransferRequests(w http.ResponseWriter, r *http.Request) {
        c := shared.GetDriver(r)
        if c == nil {
                shared.WriteErr(w, 401, "غير مصرح")
                return
        }
        rows, err := shared.DB.Query(`SELECT id, from_zone_id, to_zone_id, status, reason, requested_at, reviewed_at, admin_note
                FROM zone_transfer_requests WHERE driver_id = $1 ORDER BY requested_at DESC`, c.DriverID)
        if err != nil {
                shared.WriteErr(w, 500, "خطأ في الخادم")
                return
        }
        defer rows.Close()
        ...
        for rows.Next() {
                var id string
                var fromZone, toZone, status, reason, adminNote sql.NullString
                var requestedAt, reviewedAt sql.NullString
                rows.Scan(&id, &fromZone, &toZone, &status, &reason, &requestedAt, &reviewedAt, &adminNote)
                list = append(list, map[string]interface{}{
                        "id": id, "fromZoneId": fromZone.String, "toZoneId": toZone.String,
                        "status": status.String, "reason": reason.String,
                        "requestedAt": requestedAt.String, "reviewedAt": reviewedAt.String, "adminNote": adminNote.String,
                })
        }
        shared.WriteJSON(w, 200, map[string]interface{}{"requests": list})
}
