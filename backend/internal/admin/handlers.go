...
func HandleAdminGetZones(w http.ResponseWriter, r *http.Request) {
        rows, _ := shared.DB.Query("SELECT id, name, name_ar, center_lat, center_lng, radius_m, polygon_geojson, color, is_active, created_at FROM delivery_zones ORDER BY created_at ASC")
        var zones []map[string]interface{}
        for rows.Next() {
                var id, name, nameAr, polygon, color sql.NullString; var lat, lng sql.NullFloat64; var rad sql.NullInt64; var active sql.NullBool; var ct sql.NullString
                rows.Scan(&id, &name, &nameAr, &lat, &lng, &rad, &polygon, &color, &active, &ct)
                zones = append(zones, map[string]interface{}{
                        "id": id.String, "name": name.String, "nameAr": nameAr.String,
                        "centerLat": lat.Float64, "centerLng": lng.Float64, "radiusM": rad.Int64,
                        "polygonGeoJson": polygon.String, "color": color.String,
...
func HandleAdminCreateZone(w http.ResponseWriter, r *http.Request) {
        var b struct {
                Name, NameAr, Color, PolygonGeoJson string
        ...
        shared.DB.Exec("INSERT INTO delivery_zones (id, name, name_ar, center_lat, center_lng, radius_m, polygon_geojson, color, is_active) VALUES ($1, $2, $3, $4, $5, $6, NULLIF($7, ''), NULLIF($8, ''), TRUE)",
                id, b.Name, b.NameAr, b.CenterLat, b.CenterLng, b.RadiusM, b.PolygonGeoJson, b.Color)
        realtime.NotifyAdminZoneUpdate(map[string]interface{}{"action": "created", "zone_id": id})
        shared.WriteJSON(w, 201, map[string]interface{}{"id": id})
}
func HandleAdminUpdateZone(w http.ResponseWriter, r *http.Request) {
        ...
        json.NewDecoder(r.Body).Decode(&b)
        shared.DB.Exec(`UPDATE delivery_zones SET
                name = COALESCE(NULLIF($1, ''), name),
                name_ar = COALESCE(NULLIF($2, ''), name_ar),
                center_lat = COALESCE(NULLIF($3, 0), center_lat),
                center_lng = COALESCE(NULLIF($4, 0), center_lng),
                radius_m = COALESCE(NULLIF($5, 0), radius_m),
                polygon_geojson = COALESCE(NULLIF($6, ''), polygon_geojson),
                color = COALESCE(NULLIF($7, ''), color)
                WHERE id = $8`,
                b.Name, b.NameAr, b.CenterLat, b.CenterLng, b.RadiusM, b.PolygonGeoJson, b.Color, id)
        if b.IsActive != nil { shared.DB.Exec("UPDATE delivery_zones SET is_active = $1 WHERE id = $2", *b.IsActive, id) }
        realtime.NotifyAdminZoneUpdate(map[string]interface{}{"action": "updated", "zone_id": id})
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}
...
func HandleAdminDeleteTier(w http.ResponseWriter, r *http.Request) {
        id := r.PathValue("id")
        var inUse int
        shared.DB.QueryRow("SELECT COUNT(*) FROM drivers WHERE tier_id = $1", id).Scan(&inUse)
        if inUse > 0 {
                shared.DB.Exec("UPDATE driver_tiers SET is_active = FALSE WHERE id = $1", id)
                shared.WriteJSON(w, 200, map[string]interface{}{"success": true, "softDeleted": true, "reason": "المستوى مرتبط بمندوبين حاليين، تم تعطيله بدل الحذف"})
                return
        }
        shared.DB.Exec("DELETE FROM tier_zone_prices WHERE tier_id = $1", id)
        shared.DB.Exec("DELETE FROM tier_thresholds WHERE tier_id = $1", id)
        shared.DB.Exec("DELETE FROM driver_tiers WHERE id = $1", id)
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true})
}

// ===== ADMIN: ZONE TRANSFER REQUESTS =====
func HandleAdminGetZoneTransferRequests(w http.ResponseWriter, r *http.Request) {
        status := r.URL.Query().Get("status")
        q := `SELECT ztr.id, ztr.driver_id, d.name, ztr.from_zone_id, ztr.to_zone_id, ztr.status, ztr.reason,
                     ztr.requested_at, ztr.reviewed_at, ztr.admin_note
              FROM zone_transfer_requests ztr LEFT JOIN drivers d ON d.id = ztr.driver_id`
        args := []interface{}{}
        if status != "" {
                q += " WHERE ztr.status = $1"
                args = append(args, status)
        }
        q += " ORDER BY ztr.requested_at DESC"
        ...
        for rows.Next() {
                var id, driverID, driverName, fromZone, toZone, status, reason, adminNote sql.NullString
                var requestedAt, reviewedAt sql.NullString
                rows.Scan(&id, &driverID, &driverName, &fromZone, &toZone, &status, &reason, &requestedAt, &reviewedAt, &adminNote)
                list = append(list, map[string]interface{}{
                        "id": id.String, "driverId": driverID.String, "driverName": driverName.String,
                        "fromZoneId": fromZone.String, "toZoneId": toZone.String, "status": status.String,
                        "reason": reason.String, "requestedAt": requestedAt.String, "reviewedAt": reviewedAt.String,
...
func HandleAdminReviewZoneTransferRequest(w http.ResponseWriter, r *http.Request) {
        id := r.PathValue("id")
        c := shared.GetUser(r)
        var b struct {
        ...
        var driverID, toZoneID, status string
        err := shared.DB.QueryRow("SELECT driver_id, to_zone_id, status FROM zone_transfer_requests WHERE id = $1", id).Scan(&driverID, &toZoneID, &status)
        if err != nil {
                shared.WriteErr(w, 404, "الطلب غير موجود")
                return
        }
        if status != "pending" {
                shared.WriteErr(w, 400, "تم مراجعة هذا الطلب من قبل")
                return
        }
        reviewerID := ""
        if c != nil {
        ...
        if b.Approve {
                newStatus = "approved"
                shared.DB.Exec("UPDATE drivers SET zone_id = $1 WHERE id = $2", toZoneID, driverID)
        }
        shared.DB.Exec(`UPDATE zone_transfer_requests SET status = $1, reviewed_at = CURRENT_TIMESTAMP, reviewed_by = $2, admin_note = $3 WHERE id = $4`,
                newStatus, reviewerID, b.AdminNote, id)
        realtime.NotifyDriverOrderUpdate(driverID, map[string]interface{}{"type": "zone_transfer_reviewed", "status": newStatus, "zone_id": toZoneID})
        shared.WriteJSON(w, 200, map[string]interface{}{"success": true, "status": newStatus})
}
